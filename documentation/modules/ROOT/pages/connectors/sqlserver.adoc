...
Debezium emits a new message to this topic whenever a new table is streamed from or when the structure of the table is altered ({link-prefix}:{link-sqlserver-connector}#sqlserver-schema-evolution[schema evolution procedure must be followed]).
...
* `ddl` contains the DDL responsible for the schema change (not available for SQL Server connector)
...
[[deploying-the-sqlserver-connector]]
...
[[sqlserver-monitoring-snapshots]]
...
[[sqlserver-monitoring-streaming]]
...
[[sqlserver-monitoring-schema-history]]
...
|[[connector-property-source-timestamp-mode]]<<connector-property-source-timestamp-mode>>, `source.timestamp.mode`
|_commit_
